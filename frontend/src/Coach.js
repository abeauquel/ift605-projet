  ...
  { field: 'userName', headerName: 'Nom d\'utilisateur', width: 150 },
  {
    ...
    renderCell: (params) => {
        ...
        const onDelete = (e) => {
          ...
          fetch(ApiUrl + "/client/" + user.id + "/coach/" + params.row.id, {
              "method": "DELETE",
              "headers": {
...
function CustomToolbar() {
    ...
    const [coachsAvailable, setCoachsAvailable] = React.useState([]);
    ...
    const handleSubmit = (event) => {
        ...
        if (coach !== '') {
            addCoach(user.id, coach);
            setOpen(false);
        }
    };

    const addCoach = (userId, coachId) => {
      fetch(ApiUrl + "/client/" + userId + "/coach/" + coachId, {
        "method": "PUT",
        "headers": {
            "content-type": "application/json",
            "accept": "application/json"
    ...
    fetch(ApiUrl + '/clients', {
      ...
      .then(response => response.json())
      .then(response => {
          setCoachsAvailable(response.filter(x => x.id !== user.id));
      })
      .catch(err => {
          NotificationManager.error('An error occurred, try again', 'Error');
      });
    ...
    return (
        <Container maxWidth="l" sx={{paddingBottom: 6.5, borderBottom: `1px solid #cccccc`}}>
            <Button onClick={handleOpen} variant="outlined" sx={{ position: "absolute", left: 1, my: 1, mx: 1 }}>Ajouter un coach</Button>
            <Modal
                open={open}
                onClose={handleClose}
